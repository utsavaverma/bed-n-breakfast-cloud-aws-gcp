...
BOOK_TOUR_API_ENDPOINT = "https://serverless-project-apigateway-8y9e544e.uc.gateway.dev/booktour"
BOOK_MEAL_API_ENDPOINT = "https://serverless-project-apigateway-8y9e544e.uc.gateway.dev/bookmeal"
BOOK_ROOM_API_ENDPOINT = "https://serverless-project-apigateway-8y9e544e.uc.gateway.dev/bookroom"
View_MENU_API_ENDPOINT = "https://61vzqlv1fa.execute-api.us-east-1.amazonaws.com/prod/menu"
VIEW_TOURS_API_ENDPOINT = "https://nue260zeb8.execute-api.us-east-1.amazonaws.com/prod/tour"
VIEW_ROOMS_API_ENDPOINT = "https://ay1h3z9wt0.execute-api.us-east-1.amazonaws.com/prod/room"
...
def viewTour(intent_request):
    """
    Performs dialog management and fulfillment for booking a tour.
    ...
    res = http.request('GET', VIEW_TOURS_API_ENDPOINT)
    print('Status: ', res.status)
    print('Response: ', res.data)
    print(json.loads(res.data)['body'])
    data = json.loads(res.data)['body']
    finalResponse = "We currently have "
    
    for i in data:
        finalResponse += " "+i['title'] + " of price: " + str(i['price']) + " , "
...
def viewRooms(intent_request):
    ...
    print(json.loads(res.data)['body'])
    data = json.loads(res.data)['body']
    finalResponse = "We currently have all the mentioned rooms- "
    
    for i in data:
        finalResponse += " Title: " + i['title'] +" room type: "+i['category'] + " with description: "+ i['description']+" of daily rate: " + str(i['dailyRate'])
...
def view_menu(intent_request):
    ...
    http = urllib3.PoolManager()
    res = http.request('GET', View_MENU_API_ENDPOINT)
    print('Status: ', res.status)
    #print('Response: ', res.data)
    print(json.loads(res.data)['body'])
    data = json.loads(res.data)['body']
    ...
    for i in data:
        finalResponse += " "+i['title'] + " price: " + str(i['price'])
...
def bookTour(intent_request):
    ...
    2) Use of sessionAttributes to pass information that can be used to guide conversation
    """
    #userid = try_ex(lambda: intent_request['currentIntent']['slots']['userid'])
    userid = intent_request['sessionAttributes']['userid']
    title = try_ex(lambda: intent_request['currentIntent']['slots']['title'])
    ...
    # Booking the room.  In a real application, this would likely involve a call to a backend service.
    # print('Tour ID: ', get_tour_id(tour_type))
    ...
    http = urllib3.PoolManager()
    res1 = http.request('GET', VIEW_TOURS_API_ENDPOINT)
    print('Status: ', res1.status)
    #print('Response: ', res.data)
    print(json.loads(res1.data)['body'])
    data = json.loads(res1.data)['body']
    price = 0
    
    for i in data:
        nameOfTour = str(i['title'].split(":"))
        finalName = nameOfTour[0].replace(" ","").lower()
        print(nameOfTour[0].replace(" ","").lower())
        #if(str(i['title']).replace(" ","").lower() == title):
        if(str(i['title']).replace(" ","").lower() == title):
            price = i['price']
    ...
    return close(
        ...
        {
            'contentType': 'PlainText',
            'content': 'Thank you for booking the tour: ' + str(title) + '. The price of '+ str(title)+ " is $" + str(price)
        }
    )

def bookMeal(intent_request):
    """
    Performs dialog management and fulfillment for booking a tour.
    ...
    http = urllib3.PoolManager()
    res1 = http.request('GET', View_MENU_API_ENDPOINT)
    print('Status: ', res1.status)
    #print('Response: ', res.data)
    print(json.loads(res1.data)['body'])
    data = json.loads(res1.data)['body']
    ...
    for i in data:
        print(str(i['title']).replace(" ","").lower())
        if(str(i['title']).replace(" ","").lower() == title):
            price = i['price']
    
    res = http.request('POST', BOOK_MEAL_API_ENDPOINT,
                 headers={'Content-Type': 'application/json'},
                 body=json.dumps(room_data))
    print('Status: ', res.status)
    print('Response: ', res.data)
    ...
    return close(
        ...
        {
            'contentType': 'PlainText',
            'content': 'Thank you for booking the meal: ' + str(title) + '. The price of '+ str(title)+ " is $" + str(price)
        }
    )
...
def book_room(intent_request):
    ...
    2) Use of sessionAttributes to pass information that can be used to guide conversation
    """
    #print(intent_request['sessionAttributes']['userid'])
    #userid = try_ex(lambda: intent_request['currentIntent']['slots']['userid'])
    userid = intent_request['sessionAttributes']['userid']
    room_type = try_ex(lambda: intent_request['currentIntent']['slots']['roomtype'])
    fromDate = try_ex(lambda: intent_request['currentIntent']['slots']['from'])
    toDate = try_ex(lambda: intent_request['currentIntent']['slots']['to'])
    email = try_ex(lambda: intent_request['currentIntent']['slots']['email'])
    ...
    res1 = http.request('GET', VIEW_ROOMS_API_ENDPOINT)
    print('Status: ', res1.status)
    #print('Response: ', res.data)
    print(json.loads(res1.data)['body'])
    data = json.loads(res1.data)['body']
    ...
    for i in data:
        print(str(i['category']).lower())
        print(room_type) 
        if(str(i['category']).lower() == str(room_type).lower): 
            price = i['dailyRate']
    res = http.request('POST', BOOK_ROOM_API_ENDPOINT,
                 headers={'Content-Type': 'application/json'},
                 body=json.dumps(room_data))
    print('Status: ', res.status)
    print('Response: ', res.data)
    ...
    return close(
        ...
        {
            'contentType': 'PlainText',
            'content': 'Thank you for booking the room of type ' + str(room_type) + ' from ' + str(fromDate) + ' till ' + str(toDate)
            # + ' having daily price of: ' + str(price)
        }
    )
    
def unauth(intent_request):
    return close(

        'Fulfilled',
        {
            'contentType': 'PlainText',
            'content': 'You must login before you can book any services of the hotel.'
        }
    )
...
def dispatch(intent_request): 
    ...
    # sessionAttributesName = intent_request['sessionAttributes']
    # print(sessionAttributesName)

    # userid = intent_request['sessionAttributes']['userid']
    # print(str(userid))
    print(intent_request['sessionAttributes']) 
    #obj = json.loads(intent_request['sessionAttributes'])
    #if(len(str(userid))==0):
    #if not intent_request['sessionAttributes']['userid']:
    if not 'userid' in intent_request['sessionAttributes']:
        if intent_name == 'ViewMenu':
            return view_menu(intent_request)
        elif intent_name == 'BookRoom':
            return unauth(intent_request)
        elif intent_name == 'ViewTours':
            return viewTour(intent_request)
        elif intent_name == 'ViewRooms':
            return viewRooms(intent_request)
        elif intent_name == 'BookAMeal':
            return unauth(intent_request)
        elif intent_name == 'BookTour':
            return unauth(intent_request)
        ...
        if intent_name == 'ViewMenu':
            ...
            return bookMeal(intent_request)
        elif intent_name == 'BookTour':
            return bookTour(intent_request)
